...
def ekatalog(pilih):
    ...
    def convert_trxdaring(dftrxdaring):
        return dftrxdaring.to_csv().encode('utf-8')
    ...
    with tab1:
        ...
        ## Cek Perhitungannya Sepertinya Salah
        tmp_daring_loc = df_daring_loc[['nama_satker', 'order_id', 'valuasi']]
        ...
        opdtrxsum_daring = tmp_daring_loc_ok.groupby(by='nama_satker').sum().sort_values(by='valuasi', ascending=False)['valuasi']  
        ##

        jumlah_trx_daring = df_daring_loc['order_id'].value_counts().shape
        nilai_trx_daring = tmp_daring_loc_ok['valuasi'].sum()
        ...
        st.download_button(
            ...
            file_name = 'trxdaromh-' + kodeRUP + '.csv',
            mime = 'text/csv'
        )
